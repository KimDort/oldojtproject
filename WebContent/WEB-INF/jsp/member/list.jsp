...
<script>
	...
	function createMember(){
		$(location).attr('href','${pageContext.request.contextPath }/member/registerForm.do?page=${cri.page}&perPageNum=${cri.perPageNum}');
	}
	...
	<div class="row">
		...
		<table class="table table-hover" style="width: 100%;">
			...
			<tbody>
				...
				<c:set var="skill" value="${fn:split(idx.member_haveskill, ',') }"/>
				<tr>				
					<td>${idx.member_no }</td>
					<td>
						<a href="${pageContext.request.contextPath }/member/read.do?page=${cri.page}&perPageNum=${cri.perPageNum}&member=${idx.member_no}">${idx.member_name }</a>
					</td>
					<td align="center">
					<c:set var="totalYear" value="0"/>
					<c:set var="totalMonth" value="0"/>
						<c:forEach items="${career }" var="car">
							<c:if test="${idx.member_no eq car.mno }">
						...
						<a data-toggle="modal" data-target="#period" onclick="period(${idx.member_no})">
					...
					<td>${idx.member_rank }</td>
					<td>${idx.member_join }</td>
					<td>
						<c:set var="number" value="${fn:split(idx.member_idnumber,'-') }"/>
					...
					<td>${idx.member_isnew }</td>
					<td><input type="button" class="btn btn-default" value="Modify" onclick="modifyMember(${idx.member_no})"></td>
					<td><input type="button" class="btn btn-default" value="Delete" onclick="deleteMember(${idx.member_no})"></td>
				</tr>
				</c:forEach>
			</tbody>
			<tfoot>		
